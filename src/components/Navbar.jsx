            ...
            {navigation.map((item, index) => (
              <div key={index} className="relative group">
                ...
                >
                  {item.name}
                  {item.children && (
                    <svg className="w-3 h-3 text-gray-400 group-hover:text-blue-500 transition-transform group-hover:rotate-180 duration-200" 
                ...
                {/* DROPDOWN DESKTOP */}
                {item.children && (
                  <div className="absolute left-0 mt-1 w-64 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform translate-y-1 group-hover:translate-y-0">
      ...
      <div className={`lg:hidden transition-all duration-300 ease-in-out ${isOpen ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
        <div className="px-2 pt-2 pb-3 space-y-1 bg-white border-t border-gray-100">
          ...
          <div className="pt-2 px-3">
            <button className="w-full flex items-center gap-2 px-3 py-2.5 rounded-md text-gray-700 hover:text-blue-600 hover:bg-gray-50 transition-colors">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
              </svg>
